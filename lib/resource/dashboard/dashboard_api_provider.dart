...
//        'user_id': userId,
        'user_id': 'teguh', //todo remove dummy teguh
        'division_id': divisionId
      }
    };
    var body = json.encode(data);

      final response = await client.post("$_baseUrl/dashboardtm/tasks",
          headers: {"Content-Type": "application/json"}, body: body).catchError((error, stackTrace) {
        return TaskDashboardModel.error('Error occurred while Communication with Server. ${error.toString()}\n${stackTrace.toString()}');
      });
      App.alice.onHttpResponse(response);

      if (response == null) {
        return TaskDashboardModel.error('Unexpected for empty result, please try again later');
      }
      ...
      if (response.statusCode != 200) {
        return TaskDashboardModel.error("Unexpected (" + response.statusCode.toString() + ") for empty result, please try again later");
      }
